...
DuckDB is integral to my workflow. I use it when `polars` is not enough. Because DuckDB can easily query `parquet` we're good to go in most
Machine Learning setups. 

Connecting directly to S3 is as simple as using an [extension](https://duckdb.org/docs/guides/import/s3_import.html) which is done in the following way:
